...
try:
	...
	try:
		if os.name == 'posix':
			...
			sys.exit('[+] necessary modules have installed.')
		elif os.name == 'nt':
...
'''
	...
	options.add_argument('-d',metavar='<ip|domain>',default=False,help='specifiy target ip or domain address')
	...
	if args.Synflood:
		uid = os.getpid()
		if uid == 0:
			cprint('[*] you have permisson to run this script','green')
			time.sleep(0.5)
		else:
			sys.exit(cprint('[-] you don\'t have permission to run this script','red'))
		tgt=check_tgt(args)
